"""
Train LSTM, BiLSTM, and Transformer models for election prediction
Then compare results and predict 2573
"""
import json
import os
import numpy as np
...
class LSTMModel(nn.Module):
    def __init__(self, input_size, hidden_size=64, num_layers=2, num_classes=4, dropout=0.3):
        ...
        self.fc_reg = nn.Sequential(
            ...
            nn.Linear(32, 3)  # 3 alignment shares
        )
...
if __name__ == "__main__":
    print("=" * 60)
    print("Training AI Models for Election Prediction")
    print("=" * 60)
    ...
    lstm, lstm_hist = train_model(lstm, train_loader, test_loader, "lstm")
    results["LSTM"] = evaluate_model(lstm, X_test, y_reg_test, y_cls_test)
    print(f"    ✅ Accuracy: {results['LSTM']['accuracy']:.1f}% | MAE: {results['LSTM']['mae']:.2f} | R²: {results['LSTM']['r2']:.3f}")
    
    # Train BiLSTM
    print("\n[3] Training BiLSTM...")
    bilstm = BiLSTMModel(input_size)
    bilstm, bilstm_hist = train_model(bilstm, train_loader, test_loader, "bilstm")
    results["BiLSTM"] = evaluate_model(bilstm, X_test, y_reg_test, y_cls_test)
    print(f"    ✅ Accuracy: {results['BiLSTM']['accuracy']:.1f}% | MAE: {results['BiLSTM']['mae']:.2f} | R²: {results['BiLSTM']['r2']:.3f}")
    ...
    results["Transformer"] = evaluate_model(transformer, X_test, y_reg_test, y_cls_test)
    print(f"    ✅ Accuracy: {results['Transformer']['accuracy']:.1f}% | MAE: {results['Transformer']['mae']:.2f} | R²: {results['Transformer']['r2']:.3f}")
    ...
    print("=" * 60)
    print(f"\n{'Model':<15} {'Accuracy':>10} {'MAE':>8} {'RMSE':>8} {'R²':>8}")
    print("-" * 50)
    for name, res in results.items():
        print(f"{name:<15} {res['accuracy']:>9.1f}% {res['mae']:>8.2f} {res['rmse']:>8.2f} {res['r2']:>8.3f}")
    
    # Best model
    best_model_name = max(results, key=lambda k: results[k]["accuracy"])
    print(f"\n🏆 Best model: {best_model_name} (Accuracy: {results[best_model_name]['accuracy']:.1f}%)")
    
    # ============================================================
    # Predict 2573
    # ============================================================
    print("\n[5] Predicting 2573 election results...")
    ...
    for model_name, model in [("LSTM", lstm), ("BiLSTM", bilstm), ("Transformer", transformer)]:
        model.eval()
        with torch.no_grad():
            pred_reg, pred_cls = model(torch.FloatTensor(X_pred_scaled))
        ...
        predictions_2573[model_name] = model_preds
    ...
    output = {
        ...
        "training_history": {
            "LSTM": {"train_loss": lstm_hist["train_loss"], "val_loss": lstm_hist["val_loss"], "val_acc": lstm_hist["val_acc"]},
            "BiLSTM": {"train_loss": bilstm_hist["train_loss"], "val_loss": bilstm_hist["val_loss"], "val_acc": bilstm_hist["val_acc"]},
            "Transformer": {"train_loss": trans_hist["train_loss"], "val_loss": trans_hist["val_loss"], "val_acc": trans_hist["val_acc"]},
        },
    ...
    print("\n" + "=" * 60)
    print(f"2573 Predictions (using {best_model_name})")
    print("=" * 60)
    
    best_preds = predictions_2573[best_model_name]
    winner_dist = Counter(p["predicted_winner"] for p in best_preds)
